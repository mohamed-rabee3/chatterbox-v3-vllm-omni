...
class S3Token2Mel(torch.nn.Module):
    ...
    def forward(
        ...
        noised_mels=None,
        generator=None,  # PORT: request-scoped RNG
    ):
        """
        ...
        output_mels, _ = self.flow.inference(
            ...
            meanflow=self.meanflow,
            generator=generator,  # PORT: request-scoped RNG
            **ref_dict,
        )
        return output_mels
...
class S3Token2Wav(S3Token2Mel):
    ...
    @torch.inference_mode()
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None, generator=None):  # PORT: generator
        if cache_source is None:
            cache_source = torch.zeros(1, 1, 0).to(device=self.device, dtype=self.dtype)
        return self.mel2wav.inference(speech_feat=speech_feat, cache_source=cache_source, generator=generator)
    ...
    def inference(
        ...
        output_wavs, output_sources = self.hift_inference(output_mels, None, generator=generator)  # PORT
