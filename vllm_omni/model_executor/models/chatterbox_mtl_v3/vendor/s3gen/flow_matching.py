...
class CausalConditionalCFM(ConditionalCFM):
    ...
    def forward(self, mu, mask, n_timesteps, temperature=1.0, spks=None, cond=None, noised_mels=None, meanflow=False,
                generator=None, noise=None):  # PORT: request-scoped RNG / explicit noise
        """Forward diffusion
        ...
        B = mu.size(0)
        # PORT: request-scoped flow noise. `noised_mels` alone only replaces the
        # generated region of this tensor, so it does NOT make the solver
        # deterministic; the prompt region still came from the global RNG.
        # `generator=None` keeps the exact upstream draw for reference parity.
        # PORT: an explicit `noise` tensor takes precedence. Ragged batching
        # needs each row's noise to be the one it would have drawn ALONE, but a
        # draw over the padded batch shape depends on the padding, so the caller
        # assembles the tensor per row and passes it in whole.
        if noise is not None:
            if tuple(noise.shape) != tuple(mu.shape):
                raise ValueError(f"noise shape {tuple(noise.shape)} != mu shape {tuple(mu.shape)}")
            z = noise.to(device=mu.device, dtype=mu.dtype)
        elif generator is None:
            z = torch.randn_like(mu)
        else:
            from .hifigan import _draw  # PORT: shared per-row generator contract

            z = _draw(mu.shape, mu, generator)
