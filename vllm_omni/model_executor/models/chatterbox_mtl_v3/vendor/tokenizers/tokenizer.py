...
class MTLTokenizer:
    def __init__(self, vocab_file_path):
        self.tokenizer: Tokenizer = Tokenizer.from_file(vocab_file_path)
        self._model_dir = Path(vocab_file_path).parent
        # PORT: the Cangjie converter is only needed for Chinese, but upstream
        # builds it in __init__ -- which reaches the Hugging Face Hub for
        # Cangjie5_TC.json and initialises a segmenter on EVERY tokenizer
        # construction. On a server that is a network call and hundreds of
        # milliseconds on the request path for languages that never use it.
        # Built lazily instead; behaviour for zh is unchanged.
        self._cangjie_converter = None
        self.check_vocabset_sot_eot()

    @property
    def cangjie_converter(self):  # PORT: lazy, see __init__
        if self._cangjie_converter is None:
            self._cangjie_converter = ChineseCangjieConverter(self._model_dir)
        return self._cangjie_converter
